-- LUA-Horoskop-Plugin
-- Lizenz: GPL 2
-- Version 0.01

-- Eigenen Pfad ermitteln
function script_path()
	local str = debug.getinfo(2, "S").source:sub(2)
	return str:match("(.*/)")
end

-- ####################################################################
-- function from http://lua-users.org/wiki/BaseSixtyFour

-- character table string

local b='ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'

-- decode
function dec(data)
	data = string.gsub(data, '[^'..b..'=]', '')
	return (data:gsub('.', function(x)
    	if (x == '=') then return '' end
    	local r,f='',(b:find(x)-1)
    	for i=6,1,-1 do r=r..(f%2^i-f%2^(i-1)>0 and '1' or '0') end
    	return r;
	end):gsub('%d%d%d?%d?%d?%d?%d?%d?', function(x)
    	if (#x ~= 8) then return '' end
    	local c=0
    	for i=1,8 do c=c+(x:sub(i,i)=='1' and 2^(8-i) or 0) end
    	return string.char(c)
	end))
end
-- ####################################################################

function decodeImage(b64Image)
	local imgTyp = b64Image:match("data:image/(.-);base64,")
	local repData = "data:image/" .. imgTyp .. ";base64,"
	local b64Data = string.gsub(b64Image, repData, "");

	local tmpImg = os.tmpname()
	local retImg = tmpImg .. "." .. imgTyp

	local f = io.open(retImg, "w+")
	f:write(dec(b64Data))
	f:close()
	os.remove(tmpImg)

	return retImg
end


-- Dekleration
baseUrl = "https://www.krone.at/horoskop-" -- z.B. https://www.krone.at/horoskop-widder
tmpPath = "/tmp/horoskop"

horoskop = {}

horoskop[1] = "Widder" 
horoskop[2] = "Stier"
horoskop[3] = "Zwillinge"
horoskop[4] = "Krebs"
horoskop[5] = "Loewe"
horoskop[6] = "Jungfrau"
horoskop[7] = "Waage"
horoskop[8] = "Skorpion"
horoskop[9] = "Schuetze"
horoskop[10] = "Steinbock"
horoskop[11] = "Wassermann"
horoskop[12] = "Fische"

-- Umlaute umwandeln
function conv_str(_string)
	if _string == nil then return _string end
        _string = string.gsub(_string,'\\%s+',' ')
        _string = string.gsub(_string,'<p>','&nbsp;')
        _string = string.gsub(_string,'Loewe','Löwe')
        _string = string.gsub(_string,'Schuetze','Schütze')
        _string = string.gsub(_string,'%. ','.\n')
        _string = string.gsub(_string,'%! ','!\n')
        _string = string.gsub(_string,'%? ','?\n')
        _string = string.gsub(_string,'So stehen die Sterne heute für das Sternzeichen','Tageshoroskop für das Sternzeichen ') 
        _string = string.gsub(_string,'</strong> ','\n')
        _string = string.gsub(_string,'<strong>','\n')
        _string = string.gsub(_string,'<.->','')
        _string = string.gsub(_string,'&nbsp;','')
	return _string
end

function getdata(Url,outputfile)
	if Url == nil then return nil end
	if Curl == nil then
		Curl = curl.new()
	end
	local ret, data = Curl:download{url=Url,A="Mozilla/5.0;",followRedir=true,o=outputfile }

	if ret == CURL.OK then
		return data
	else
		return nil
	end
end

-- Funktionen
function auswahl(k)
	horoskop_icon = decodeImage("data:image/png;base64,")
	s = tonumber(k)
	kreis = horoskop[s]
	local h = hintbox.new{caption="Horoskop-Info", icon=horoskop_icon, text="Die Sterne werden gelesen...\nBitte warten ..."}
	h:paint()
	os.execute("sleep 3")
	h:hide()

	local data = getdata( baseUrl .. kreis ,nil)
		if data then
                        local horoskoptitel,horoskoptext = data:match('daily%-horoscope%-content__headline">(.-)</h2>.-<div class="daily%-horoscope%-content__text">.-<strong>(.-)<div class="daily%-horoscope%-content__bottom">')
			if horoskoptext then
	                        horoskoptitel = conv_str(horoskoptitel)
	                        horoskoptext = conv_str(horoskoptext)
			        local ret = messagebox.exec{ icon="/var/tuxbox/plugins/horoskop_klein.png",title=horoskoptitel, text=horoskoptext, width="550", text_mode="ALIGN_CENTER | ALIGN_AUTO_HIGH | DECODE_HTML", buttons={"ok"}, timeout="0" };
			end
                 end
end

-- Menueanzeige
horoskop_icon = decodeImage("data:image/png;base64,")
local m = menu.new{name="Horoskop", icon=horoskop_icon}
--m:addItem{type="back"}
--m:addItem{type="separatorline"}

local i
for i = 1, 12 do
        if (i == 1) then
		key = "red"
		icon = "rot"
	elseif (i == 2) then
		key = "green"
		icon = "gruen"
	elseif (i == 3) then
		key = "yellow"
		icon = "gelb"
	elseif (i == 4) then
		key = "blue"
		icon = "blau"
	else
		j = i - 4
		key = tostring (j)
		icon = tostring (j)
	end
	m:addItem{type="forwarder", name=horoskop[i], action="auswahl", id=i, icon=icon, hint="Hier werden die Sterne für das Sternzeichen " .. horoskop[i] .. " befragt", hint_icon="horoskop_hint", directkey=RC[key]}
end
m:exec()
os.execute("rm /tmp/lua*.png");
