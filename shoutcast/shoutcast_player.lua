...
function init()
	...
	stream = 1
        ...
        shoutcast_dev_id = "fa1669MuiRPorUBw" -- Please enter your own shoutcast dev-key, avoids overloading fa1669MuiRPorUBw
end
...
function fill_playlist(id)
	...
	for i,v in  pairs(subs) do
		if v[1] == id then
            ...
            if data then
                for title,seite,genre,logo in data:gmatch('<station% name="(.-)"% mt.-id="(.-)".-genre="(.-)".-logo="(.-)".-/>') do 
                      title = uml_str(title)
                      if seite and title then
                       	   url = 'http://yp.shoutcast.com/sbin/tunein-station.pls?id=' .. seite .. '&k=' .. shoutcast_dev_id 
...
function select_playitem()
  ...
  repeat
	...
	local url = js_data:match('File1=(.-)\n.-Title1')
        title = p[pmid].title

	if url then
    			vPlay:ShowPicture("radiomode.jpg")
    			vPlay:PlayFile("shoutcast",url, uml_str(title), url); -- default
